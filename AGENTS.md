...
- **対症療法で指摘を消さない（厳守）**。修正に着手する前に、破られた不変条件を1文で言えるようにする。症状だけを黙らせる変更（分岐の追加、閾値・期待値の調整、例外の握り潰し、テストの緩和・skip、特定ケースの特別扱い）は、根本原因を特定し他に同型の欠陥がないと確認したうえでなければ採らない。根本対処がスコープに収まらないときは、対症療法である事実・根本原因・残る影響範囲を明示して差し戻す。黙って完了扱いにしない
...
定型手法を採る前に、**その厳密さを行使する人がこのプロジェクトにいるか**を確かめる。いなければ採らない。広く使われていることも、影響範囲の広いソフトでの定石であることも、ここで必要な理由にならない。

- **行使されない保証のために、人が日常的に使う情報を捨てない**。日付・名前・履歴を消す前に、消して誰が何を得るかを言えるようにする
- 迷ったら、その保証を行使する手順が実在するかを確認する。無ければ過剰
